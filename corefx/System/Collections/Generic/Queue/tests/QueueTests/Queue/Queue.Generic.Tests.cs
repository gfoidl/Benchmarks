...
{
    /// <summary>
    /// Contains tests that ensure the correctness of the Queue1 class.
    /// </summary>
    public abstract partial class Queue_Generic_Tests<T> : IGenericSharedAPI_Tests<T>
    {
        #region Queue1<T> Helper Methods

        protected Queue1<T> GenericQueueFactory()
        {
            return new Queue1<T>();
        }

        protected Queue1<T> GenericQueueFactory(int count)
        {
            Queue1<T> queue = new Queue1<T>(count);
            int seed = count * 34;
        ...
        protected override int Count(IEnumerable<T> enumerable) => ((Queue1<T>)enumerable).Count;
        protected override void Add(IEnumerable<T> enumerable, T value) => ((Queue1<T>)enumerable).Enqueue(value);
        protected override void Clear(IEnumerable<T> enumerable) => ((Queue1<T>)enumerable).Clear();
        protected override bool Contains(IEnumerable<T> enumerable, T value) => ((Queue1<T>)enumerable).Contains(value);
        protected override void CopyTo(IEnumerable<T> enumerable, T[] array, int index) => ((Queue1<T>)enumerable).CopyTo(array, index);
        protected override bool Remove(IEnumerable<T> enumerable) { ((Queue1<T>)enumerable).Dequeue(); return true; }
        ...
        public void Queue_Generic_Constructor_IEnumerable(EnumerableType enumerableType, int setLength, int enumerableLength, int numberOfMatchingElements, int numberOfDuplicateElements)
        {
            ...
            Queue1<T> queue = new Queue1<T>(enumerable);
        ...
        {
            AssertExtensions.Throws<ArgumentNullException>("collection", () => new Queue1<T>(null));
        }
        ...
        {
            AssertExtensions.Throws<ArgumentOutOfRangeException>("capacity", () => new Queue1<T>(-1));
            AssertExtensions.Throws<ArgumentOutOfRangeException>("capacity", () => new Queue1<T>(int.MinValue));
        }
        ...
        {
            Queue1<T> queue = GenericQueueFactory(count);
        ...
        {
            Assert.Throws<InvalidOperationException>(() => new Queue1<T>().Dequeue());
        }
        ...
        {
            Queue1<T> collection = new Queue1<T>(count + 1);
        ...
        {
            Assert.Throws<InvalidOperationException>(() => new Queue1<T>().Peek());
        }
