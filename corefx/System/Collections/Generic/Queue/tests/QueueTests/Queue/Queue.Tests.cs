    ...
    public class Queue_ICollection_NonGeneric_Tests : ICollection_NonGeneric_Tests
    {
        ...
        protected override void AddToCollection(ICollection collection, int numberOfItemsToAdd)
        {
            ...
            for (int i = 0; i < numberOfItemsToAdd; i++)
                ((Queue1<string>)collection).Enqueue(CreateT(seed++));
        }

        protected override ICollection NonGenericICollectionFactory()
        {
            return new Queue1<string>();
        }
        ...
        protected override IEnumerable<ModifyEnumerable> ModifyEnumerables
        {
            get
            {
                ...
                {
                    var casted = (Queue1<string>)enumerable;
        ...
        private static IEnumerable<object[]> GenerateCollections(bool includeEmptyCollections)
        {
            ...
            foreach (int size in sizes)
            {
                ...
                yield return new object[] { new Queue1<int>(Enumerable.Range(0, size).ToList()) };
        ...
        private static Queue1<T> CreateQueueAtCapacity<T>(bool initializeFromCollection, Func<int, T> selector, int size)
        {
            Queue1<T> q;

            if (initializeFromCollection)
            {
                q = new Queue1<T>(Enumerable.Range(0, size).Select(selector).ToArray());
            }
            else
            {
                q = new Queue1<T>(size);
        ...
        public void Clear_Wrapped(bool initializeFromCollection)
        {
            ...
            Queue1<string> q = CreateQueueAtCapacity(initializeFromCollection, i => i.ToString(), size: 4);
        ...
        public void CopyTo_Wrapped(bool initializeFromCollection)
        {
            ...
            Queue1<int> q = CreateQueueAtCapacity(initializeFromCollection, i => i, size: 4);
            Assert.Equal(0, q.Dequeue());
            Assert.Equal(1, q.Dequeue());
