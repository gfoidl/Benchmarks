...
namespace System.Collections.Generic
{
	...
	internal struct LargeArrayBuilder2<T>
	{
		...
		public void AddRange(IEnumerable<T> items)
		{
			...
			using (IEnumerator<T> enumerator = items.GetEnumerator())
			{
				...
				while (enumerator.MoveNext())
				{
					...
					{
						AddWithBufferAllocation(enumerator, ref destination, ref index);
					}
					else
					{
		...
		// Non-inline to improve code quality as uncommon path
		[MethodImpl(MethodImplOptions.NoInlining)]
		private void AddWithBufferAllocation(IEnumerator<T> enumerator, ref T[] destination, ref int index)
		{
			_count += index - _index;
			_index = index;
			AllocateBuffer();
			...
			index = _index;
			_current[index] = enumerator.Current;
		}
