...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Internal.Infrastructure
{
    internal class StringUtilities
    {
        ...
        public static unsafe bool TryGetAsciiStringDefault(byte* input, char* output, int count)
        {
        ...
        public static unsafe bool TryGetAsciiString(byte* input, char* output, int count)
        {
            ...
            if (Sse2.IsSupported)
            {
                if (Avx2.IsSupported && input <= end - Vector256<sbyte>.Count)
                {
                    ...
                    do
                    {
                        ...
                        isValid &= CheckBytesInAsciiRange(vector, zero);
            ...
            if (Environment.Is64BitProcess) // Use Intrinsic switch for branch elimination
            {
                ...
                while (input <= end - sizeof(long))
                {
                    ...
                    isValid &= CheckBytesInAsciiRange(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public static unsafe bool TryGetAsciiStringExitEarly(byte* input, char* output, int count)
        {
            var end = input + count;
